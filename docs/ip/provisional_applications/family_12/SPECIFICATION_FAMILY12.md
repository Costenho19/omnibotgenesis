...
5. Operates in configurable fail mode: by default, any TIE module failure produces a pass-through result that does not block the upstream pipeline decision (fail-open); in critical-domain deployments, TIE may be configured to fail-closed, treating any module failure as a blocking HOLD to prevent ungoverned decisions in high-stakes environments.
...
### V. CONFIGURABLE FAIL MODE — FAIL-OPEN AND FAIL-CLOSED

The TIE supports two configurable failure modes to accommodate deployments with different risk profiles:

**Fail-Open (default):** Any exception, database connectivity failure, or internal error within the TIE produces a pass-through result in which the upstream pipeline's APPROVED decision is returned unchanged. The pass-through reason is recorded in the TIE result for audit purposes. This mode ensures that TIE's failure mode is identical to its absence — the governance pipeline continues to operate as if TIE were not present, preserving the safety properties of the upstream pipeline without adding new failure modes. Fail-open is the recommended default for domains where decision continuity is preferred over conservative blocking.

**Fail-Closed (critical domains):** In high-stakes deployment environments — including but not limited to clinical decision support, autonomous systems with physical actuation, energy grid dispatch, and defense-grade governance — TIE may be configured to fail-closed (in one embodiment: via `TIE_FAIL_MODE=closed` environment variable). In fail-closed mode, any TIE module failure, database unavailability, or internal exception produces a HOLD result rather than a pass-through. This ensures that no APPROVED decision proceeds without a successful trajectory invariant evaluation in domains where an ungoverned decision carries unacceptable risk.

The choice between fail-open and fail-closed is a deployment configuration parameter, not an architectural constraint. Both modes record the fail-event with its reason in the TIE result for full audit traceability.
...
**7.1 Extensible Invariant Registry:** The five invariants described above represent one embodiment. The invariant set is explicitly designed to be extensible: new invariants may be registered at deployment time without modifying the core TIE engine. Each invariant is defined by a window size, a threshold, a severity classification (HOLD or WARNING), and an evaluation function over the trajectory state history. In other embodiments, additional trajectory-level invariants may be defined and registered, including but not limited to: monotonic decline in trend_persistence across K consecutive decisions; joint threshold violations across correlated pairs of metrics (e.g., simultaneous decline in logic_consistency and signal_coherence); time-weighted decay invariants that apply exponentially decaying weights to historical states; domain-specific invariants defined by regulatory mandate (e.g., maximum consecutive approvals in a single asset class for financial governance, or maximum consecutive actuations for robotic governance); and cross-domain invariants that correlate trajectory metrics across multiple operational domains simultaneously.
...
**Claim 8 (Broad — Configurable Fail Mode)** — The system of Claim 1, wherein the trajectory invariant engine supports a configurable failure mode comprising: a fail-open mode in which any operational failure produces a pass-through result returning the upstream pipeline decision unchanged; and a fail-closed mode in which any operational failure produces a HOLD result blocking the upstream pipeline decision; wherein the failure mode is selectable per deployment as a configuration parameter to accommodate domains ranging from decision-continuity-preferred to ungoverned-decision-prohibited operational environments.
