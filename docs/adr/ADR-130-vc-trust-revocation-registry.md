...
| v2.3 | 2026-04-28 | **Bug fix + ADR-135 CBG integration note.** (1) **Bug fix (T004):** `api_governance_receipt_vc` en `gov_blueprint.py` usaba `completed_at` y `status='completed'` para el auto-lookup de `oversight_sessions` — columna y valor incorrectos según el schema real de ADR-124. Corregido a `submitted_at` y `status='SUBMITTED'`. El bloque `humanSigner` ahora se incluye correctamente en el proof del VC cuando existe una sesión de oversight para el receipt. (2) **CBG integration note:** ADR-135 Conditional Bind Gate opera como capa de enforcement post-ADR-130: cuando el SPG (ADR-133) retorna veredicto AMBIGUOUS con score < 50 o contradicciones ≥ 2, el CBG congela el binding de consecuencias hasta que un revisor humano ATESTE o BLOQUEE explícitamente. El `gate_id` se puede adjuntar al VC como `credentialStatus.bindGateId` para trazabilidad de la cadena de gobernanza. (3) **Tests:** 29 nuevos tests en `tests/test_vc_revocation_v2.py` cubren todos los features v2 (T001–T005) incluyendo el bug fix T004. |
