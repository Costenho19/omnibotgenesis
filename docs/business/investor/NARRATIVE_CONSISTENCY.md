# OMNIX Investor Documents - Narrative Consistency Matrix

**Last Updated:** February 17, 2026

This table tracks key phrases and messaging across all investor-facing documents to ensure narrative consistency.

---

## Core Messaging Consistency

| Element | one_pager.md | PITCH_DECK_HUB71.md | INVESTOR_FAQ.md | FOUNDATIONAL.md |
|---------|--------------|---------------------|-----------------|-----------------|
| **Opening Hook** | "Most automated systems fail not because of lack of signals, but because of inadequate governance infrastructure." | "Most automated systems fail not because of lack of signals, but because of inadequate governance infrastructure." | N/A (FAQ format) | N/A (narrative format) |
| **Platform Identity** | "Decision Governance Infrastructure for Automated Systems" | "Decision Governance Infrastructure for Automated Systems" | "Decision Governance Infrastructure for Automated Systems" | "Decision Governance Infrastructure for Automated Systems" |
| **Restraint Line** | "Most competitors optimize for entries. OMNIX optimizes for restraint." | "Most competitors optimize for entries. OMNIX optimizes for restraint." | N/A | N/A |
| **Churn Assumption** | "<3% monthly" | "<3% monthly" | N/A | N/A |
| **Market Source** | "estimate based on active wallet behavior and exchange volume data" | "estimate based on active wallet behavior and exchange volume data" | N/A | N/A |

---

## Metrics Consistency

| Metric | one_pager.md | PITCH_DECK_HUB71.md | INVESTOR_FAQ.md | FOUNDATIONAL.md |
|--------|--------------|---------------------|-----------------|-----------------|
| **89K Metric** | "Evaluation cycles vetoed (not trade opportunities)" | "Evaluation Cycles Blocked: 89,000+" | "evaluation cycles, not trade opportunities or missed profits" | "ciclos de evaluación bloqueados (no oportunidades de trading)" |
| **Drawdown (observed)** | 1.5% | 1.5% | 1.5% | 1.5% |
| **Drawdown (hard cap)** | 15% | 15% | 15% | 15% |
| **Capital Preserved** | 98.5% | 98.5% | 98.5% | 98.5% |
| **Baseline Trades** | 119 | 119 | 119 | 119 |
| **Track Record Day** | 10/30 | 10/30 (started Jan 15) | 10/30 | 10/30 |

---

## PQC Messaging Consistency

| Aspect | one_pager.md | PITCH_DECK_HUB71.md | INVESTOR_FAQ.md | FOUNDATIONAL.md |
|--------|--------------|---------------------|-----------------|-----------------|
| **PQC Claim** | "Production-integrated PQC modules" | "Post-Quantum Security (PQC): Yes" | "production-integrated post-quantum cryptography modules" | "Módulos PQC integrados en producción" |
| **Disclaimer** | "not a compliance guarantee" | "not a compliance guarantee" | "not marketed as compliance guarantee or external security certification" | "No se comercializa como garantía de compliance" |

> **Note on PQC Variance:** The claim wording varies by document detail level (acceptable). The critical element—the compliance disclaimer—is present in ALL documents.

---

## Valuation & Ask Consistency

| Element | one_pager.md | PITCH_DECK_HUB71.md | INVESTOR_FAQ.md | FOUNDATIONAL.md |
|---------|--------------|---------------------|-----------------|-----------------|
| **Raising** | $500K | $500K | $500K | $500K |
| **Equity** | 16.7% | 16.7% | 16.7% | 16.7% |
| **Pre-Money** | $2.5M-$3M | $2.5M-$3M | $2.5M-$3M | $2.5M-$3M |

---

## Document Purpose Summary

| Document | Purpose | Audience | Length |
|----------|---------|----------|--------|
| **one_pager.md** | Quick overview, email attachment | Any investor, first contact | 1 page |
| **PITCH_DECK_HUB71.md** | Hub71/Abu Dhabi pitch | MENA investors, Hub71 | 13 slides |
| **INVESTOR_FAQ.md** | Due diligence answers | Serious investors, analysts | 20 Q&As |
| **FOUNDATIONAL.md** | Deep dive, technical | Technical investors, partners | 60+ pages |

---

## Verified Consistency: February 17, 2026

All documents now share:
- Same opening hook (governance, not signals)
- Same restraint positioning (entries vs restraint)
- Same drawdown framing (15% cap / 1.5% observed)
- Same 89K clarification (evaluation cycles)
- Same PQC disclaimer (production-integrated, not compliance)
- Same platform identity (Decision Governance Infrastructure for Automated Systems)
- Same valuation ($2.5M-$3M pre-money)

---

*This matrix should be updated whenever investor documents are modified.*
