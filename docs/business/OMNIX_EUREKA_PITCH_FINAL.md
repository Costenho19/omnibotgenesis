...
## SLIDE 5 — THE TRADE THAT DIDN'T HAPPEN

**This is our product working — not failing.**

### Real Scenario: February 3, 2026 — BTC Breakout

```
Market Conditions:
  BTC Price Action:     +6% in 2 hours
  Market Sentiment:     Euphoric
  Traditional Bot:      Would execute $50,000 position

OMNIX Decision:
┌──────────────────────────────────────────────┐
│ Checkpoint 1: Probability     → PASS    ✓   │
│ Checkpoint 2: Risk Limits     → PASS    ✓   │
│ Checkpoint 3: Signal Agreement→ PASS    ✓   │
│ Checkpoint 4: Trend Confirm   → FAIL    ✗   │
│ Checkpoint 5: Stress Test     → WARNING ⚠   │
│ Checkpoint 6: Logic Check     → PASS    ✓   │
│                                              │
│ DECISION: ██ BLOCKED ██                      │
│ Reason: Insufficient trend persistence       │
│         (momentum spike, not sustained edge) │
└──────────────────────────────────────────────┘

48 Hours Later:
  BTC dropped -9%
  Traditional bot loss: -$4,500
  OMNIX exposure: $0

Capital Preserved: $50,000
```

**This is what "fail-closed" looks like in practice.** The system identified that the breakout lacked persistence — a classic trap that destroys capital.
...
**How It Works for a Prop Firm:**

```
Trader submits order
    → OMNIX Risk Gate validates in <100ms
    → APPROVE (with audit trail) or BLOCK (with full rationale)

Impact for the firm:
  -40% drawdown events
  100% audit compliance
  Zero development cost (they license, not build)
```

### SECONDARY: B2C SaaS — Individual Traders (Post-Enterprise Validation)
...
**OMNIX is built for regulated environments.** The shift from "crypto wild west" to institutional-grade infrastructure is happening here first.
...
> Every decision OMNIX makes produces this output. Fully traceable. Fully auditable. Nothing hidden.

---

## APPENDIX B — KEY DOCUMENTS FOR DUE DILIGENCE
