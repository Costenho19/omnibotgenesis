...
- **Dec 7, 2025**: YouTube Transcript Enhanced Logging V6.5.4 - Improved `get_transcript_robust()` to call Whisper directly (not nested inside yt-dlp). All 3 fallback attempts (youtube-transcript-api, yt-dlp, Whisper) now logged in `attempts[]` array. Error messages show exactly which methods failed. Added `_get_transcript_ytdlp_only()` method for cleaner separation. Fixed missing `import threading` crash.
- **Dec 7, 2025**: YouTube Transcript Robust V6.5.4 - New unified `get_transcript_robust()` method in VideoAnalyzerUltra that consolidates all YouTube transcript fallbacks (youtube-transcript-api → yt-dlp → Whisper) into single entry point. Simplified YouTube handling in `handle_message` and `handle_direct_message` from ~400 lines to ~140 lines. Better error messages show specific failure reasons to users. Location: `omnix_services/ai_service/video/analyzer.py`, `enterprise_bot.py`.
...
- **Dec 7, 2025**: Message Aggregation V6.5.4 SYNC FIX - Fixed duplicate responses caused by TWO polling systems (async + sync) processing same messages. Added sync buffers (`_sync_message_buffers`, `_sync_message_timers`, `_sync_buffer_lock`) with `threading.Timer` for the manual polling path. Both paths now use 1.5s debounce. Location: `enterprise_bot.py` (`_buffer_sync_message`, `_process_sync_aggregated_messages`).
