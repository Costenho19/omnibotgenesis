...
## Overview
OMNIX is a Decision Governance Infrastructure designed for automated systems, initially focused on digital asset trading to ensure capital preservation. It utilizes a domain-agnostic 6-checkpoint architecture to prevent errors in high-stakes decision-making. Key features include post-quantum cryptography, real-time market analysis, Non-Markovian Temporal Memory, a 6-tier Coherence Engine, Monte Carlo validation, Black Swan detection, and Kelly Criterion sizing. OMNIX aims to expand (Year 2-3+) into various financial and industrial sectors — including robotics/autonomous physical systems, supply chain, credit/lending, insurance, and energy trading — operating with a dual revenue model: B2B Decision Governance Licensing and B2C SaaS. Combined TAM: $49.7B+ across 6+ verticals.
...
OMNIX's architecture integrates an AutoTradingBot, Non-Markovian Memory Kernel, and a 6-Tier Veto System (Coherence Engine). Key features include AI Risk Guardian, Portfolio Management, Confidence-Adaptive Entry System (CAES), On-Chain Data Intelligence, and an Execution Protocol. The system supports multi-user roles, Flask and Streamlit dashboards, an Asset Quarantine System, Real-Time Latency Monitor, Price Stale Detection, and Admin Alerts. The Decision Engine incorporates an EMA Regime Signal, a Monte Carlo VETO Engine, and RMS Enforcement. The system is designed with a hexagonal architecture (V7.0) activated via the Strangler Fig pattern.
...
A standalone aiohttp web server (`omnix_core/evidence/verification_server.py`) runs alongside the Telegram bot in Railway production, providing public receipt verification endpoints with zero internal data exposure. Rate-limited to 30 req/min per IP. Endpoints: `/verify` (interactive HTML), `/api/verify/{receipt_id}`, `/api/verify/recent`, `/api/public_key`, `/api/governance/metrics`. Security: SHA-256 hash chain + Dilithium-3 PQC signatures. Legal: "NIST-standardized algorithms", "Internal dataset, not externally audited" on all public metrics. Public URL: `https://omnibotgenesis-production.up.railway.app/verify`.
