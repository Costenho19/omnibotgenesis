...
-   **Google Gemini SDK Migration V6.5.2** (Dec 5, 2025): Migrated 6 files from deprecated `google-generativeai` to new `google-genai` SDK with dual SDK support. Added `_extract_gemini_text()` helper for robust response parsing across both SDK versions. Files: ai_models.py, conversational_ai_adapter.py, video/analyzer.py, enterprise_bot.py, community_analyzer.py, main.py. Note: websockets conflict (alpaca <11 vs google-genai >=13) documented, using alpaca version.
