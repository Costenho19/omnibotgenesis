...
- `/docs/adr/` — **205 archivos ADR, último numerado ADR-205**. Fuente de verdad de arquitectura
...
| `atf_gdcl_convergence_records` | GDCL — GDCLConvergenceRecord: composite verdict multi-RSA | ADR-206 |
...
- **Norma de construcción — SIEMPRE sin excepción:** Antes de implementar cualquier cosa: (1) leer toda la documentación relevante (ADRs, RFCs, replit.md, specs), (2) consultar al arquitecto (code_review) para validar el enfoque antes de escribir código. Durante la implementación: (3) hacer todo al nivel más premium y diferenciador posible — sin atajos, sin stubs, sin placeholders, sin mocks donde puede haber código real. Al terminar: (4) documentar todo en sus sitios canónicos — ADR nuevo o actualizado, ARCHITECTURE_INDEX, replit.md si aplica, comentarios en el código. Esta norma aplica a cada feature, cada script, cada builder, cada verifier, cada report — sin excepción.
- **Norma de calidad progresiva — OMNIX SIEMPRE HACIA ARRIBA:** Todo lo que se toque en OMNIX — implementación nueva, reparación, cambio, refactor, fix, script, UI, endpoint, documento — debe quedar en un estado superior al que estaba antes. Si es nuevo: más premium y diferenciador que cualquier implementación comparable. Si es una reparación: no solo arreglar el bug sino mejorar la robustez, legibilidad y solidez del bloque completo. Si es un cambio: aprovechar para elevar el estándar del área tocada. Nunca dejar algo igual a como estaba — siempre mejor. Sin excepciones.
- **Norma absoluta — NUNCA asumir ni actuar sin preguntar:** Ante cualquier tarea, contexto, plan o instrucción que no haya sido dicha explícitamente por Harold en el mensaje actual, PREGUNTAR antes de hacer cualquier cosa. No ejecutar por iniciativa propia. No asumir que algo "quedó pendiente". No actuar sobre planes o contextos de sesiones anteriores sin confirmación explícita. Siempre preguntar primero, actuar después.
