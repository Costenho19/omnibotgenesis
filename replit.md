...
| ROAD-008 | **Filtrar recibos inválidos en `/verify`** — La página pública muestra recibos de prueba con activos `UNINTELLIGIBLE-SCENARIO` y status `Unsigned`. Daña credibilidad ante inversores. Filtrar endpoint `/api/verify/recent` para mostrar solo pares reales (BTC/USD, ETH/USD, etc.) y recibos firmados. | Archivo `.local/tasks/filtrar-recibos-invalidos-pagina-verify.md` | Alta — credibilidad inmediata |
