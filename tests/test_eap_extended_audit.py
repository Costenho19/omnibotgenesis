...
Cobertura extendida para:
  ...
  ADR-161 — Governance Policy Interoperability Layer (GPIL)
             ...
             · CRGCPolicyParameters full field coverage
...
class TestAIFallbackChainObservability:
    ...
    def test_VII3_chain_exhausted_log_format(self):
        ...
        assert "total_attempts" in line

    def test_VII4_success_log_format_includes_char_count(self):
        """SUCCESS log must include character count for observability."""
        template = "[AI] {label} SUCCESS | attempt={a} | total_attempts={t} | chars={c}"
        line = template.format(label="GEMINI", a=1, t=3, c=1842)
    ...
    def test_VII5_primary_log_is_info_fallback_log_is_warning(self):
        ...
        assert logging.INFO < logging.WARNING, (
            "INFO level must be lower severity than WARNING"
        )

    def test_VII6_skip_log_format_identifies_non_retryable_reason(self):
        """SKIP log must identify the non-retryable error category."""
        template = "[AI] {label} SKIP (non-retryable) | error={cat} | msg={msg}"
        line = template.format(label="GPT-4o-mini", cat="auth_error", msg="401 Unauthorized")
        assert "non-retryable" in line
        assert "error=" in line

    def test_VII7_retry_log_format_includes_backoff(self):
        """RETRY log must include the backoff delay for observability."""
        template = "[AI] {label} RETRY {a}/{max} | backoff={d:.2f}s"
        line = template.format(label="GEMINI", a=1, max=2, d=1.5)
        assert "RETRY" in line
        assert "backoff=" in line

    def test_VII8_exhausted_log_is_warning_not_error(self):
        """Model exhausted log = WARNING (not ERROR — ERROR is reserved for CHAIN_EXHAUSTED)."""
        template = "[AI] {label} EXHAUSTED ({n} attempts) — advancing to next model"
        line = template.format(label="GPT-4o", n=2)
