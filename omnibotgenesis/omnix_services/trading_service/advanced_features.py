...
class BlackSwanDetector:
    ...
    Note: This is a fallback implementation. When available, use
    CanonicalBlackSwanDetector from black_swan.py for production.
    """
    
    def __init__(self):
        self.threshold_sigma = 3.5  # 3.5 desviaciones estándar (legacy)
        self.kurtosis_threshold = 3.0  # Normal distribution = 3 (canonical)
        self.skewness_threshold = 0.5  # Canonical threshold
    ...
    def analyze(
        ...
        CANONICAL IMPLEMENTATION - identical to black_swan.py
        ...
        try:
            ...
            if tail_frequency > 0.1:
                detected = True
                severity = 'high'
                indicators.append('fat_tails')
            
            if extreme_frequency > 0.05:
                detected = True
                severity = 'high'
